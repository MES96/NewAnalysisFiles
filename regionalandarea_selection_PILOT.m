...
entrainfiles = dir(fullfile(dname, '*_10LO_*'));
...
for f= 1:length(entrainfiles)
    ...
    basename = strrep(entrainname{f},'_10LO_','_1LO_');
 
    entrain = load(entrainname{f}); %entrain.f.data
    base = load(basename); % base.fdata
    
    av = squeeze(mean(entrain.fdata.powspctrm,1));
...
save('Corr10LO_averages.mat','av_area','entrainname','av_cell','grand_av_e','grand_av_b','grand_av'); % CAB: added av_cell - filenames and numbers all in one cell array
