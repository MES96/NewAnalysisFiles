...
for f = 1:length(S.filelist)
    ...
    % INTERPOLATE CHANNELS (having this step before rereferencing means
    % that your data isn't rereferenced to include noisy channels)
    ...
    if S.chan.reref == 1
        ...
        EEG = pop_reref( EEG, []); % can specify in this function if you want to exclude certain channels (i.e. removings EOG channels on the cap for referencing)
    ...
    % NOTCH FILTER % removes electrical noise and harmonics 
...
if ~isempty(S.FTrej) && iscell(S.FTrej)
    ...
    for f = 1:length(S.filelist) % was just length(S.filelist)
        file = S.filelist{f};
        EEG = pop_loadset('filename',file,'filepath',S.setpath);
